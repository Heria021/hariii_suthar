...
const getProjectSize = (index: number): string => {
  const sizes = [
...
const fetcher = async (url: string) => {
  ...
  if (!res.ok) {
    ...
    (error as any).info = errorData.message;
...
const ErrorAlert: React.FC<ErrorAlertProps> = ({ error, onRetry }) => (
  <Alert variant="destructive" className="col-span-full">
    ...
    <AlertDescription className="flex items-center justify-between">
      <span>
        Failed to load projects. Please try again later. Rate limit exceeded
      </span>
      <Button
        variant="outline"
        size="sm"
...
interface ProjectCardProps {
  project: GithubRepo & { size?: string };
  size: string;
}
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
          <div className="space-y-8 sm:space-y-12">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-4 mt-8 sm:mt-0">
              ...
              <Button
                className="rounded-full px-5 py-5 sm:px-6 sm:py-6 text-sm sm:text-base w-full sm:w-auto inline-flex items-center gap-2"
                asChild
              >
                <a
                  href={`https://github.com/${config.social.github}`}
